...
genes_of_interest_df <- expression_matrix[cell_cycle_genes,rownames(cell_metadata)]
matrix_subset_of_interest <- as.matrix(t(genes_of_interest_df))
#this technically sums a true/false matrix where true = 1 and false = 0
n_cells_expressed <- colSums(matrix_subset_of_interest > 0)

#which selects values from the first vector that match values in the second vector
most_expressed <- n_cells_expressed[which(n_cells_expressed > 499)]
...
matrix_subset_of_interest <- matrix_subset_of_interest2[,names(colSums(matrix_subset_of_interest2 > 0)[colSums(matrix_subset_of_interest2 > 0) >= 500])]
...
####Hint: There will be 14 genes that pass this threshold
```{r task 1 soln}
matrix_subset_of_interest2 <- as.matrix(t(expression_matrix[which(rownames(expression_matrix) %in% cell_cycle_genes),rownames(cell_metadata)]))

matrix_subset_of_interest2 <- matrix_subset_of_interest2[,names(colSums(matrix_subset_of_interest2 > 0)[colSums(matrix_subset_of_interest2 > 0) >= 500])]
```
